...
Blockly.Python['neopixel_action'] = function(block) {
  Blockly.Python.definitions_['from_action_import_bounce'] = 'from action import bounce,cycle,wheel,rainbow_cycle';
  var value_n = Blockly.Python.valueToCode(block, 'n', Blockly.Python.ORDER_ATOMIC);
  var colour_color = block.getFieldValue('color');
  ...
  var blue = parseInt(result[3], 16);
   ...
   var code=(dropdown_action_type=='rainbow_cycle'?`${dropdown_action_type}(np,${value_wait})\n`
   : `${dropdown_action_type}(np,int(${red} * np.bright / 100), int(${green} * np.bright / 100), int(${blue} * np.bright / 100),${value_wait})\n`); 
  //var code = `${dropdown_action_type}(np,int(${red} * np.bright / 100), int(${green} * np.bright / 100), int(${blue} * np.bright / 100),${value_wait})\n`;
  return code;
};
...
Blockly.Python['neopixel_clear'] = function(block) {
  var code = 'for i in range(np.n): np[i] = (0, 0, 0); np.write()\n';
  return code;
};
